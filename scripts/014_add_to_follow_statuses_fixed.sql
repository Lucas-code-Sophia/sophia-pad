...
-- Step 2: Update existing to_follow items to to_follow_1 for backward compatibility
UPDATE public.order_items 
SET status = 'to_follow_1' 
WHERE status = 'to_follow';

-- Step 3: Check if there are any other invalid statuses and fix them
UPDATE public.order_items 
SET status = 'pending' 
WHERE status NOT IN ('pending', 'to_follow_1', 'to_follow_2', 'fired', 'completed');

-- Step 4: Now add the new constraint with all supported statuses
ALTER TABLE public.order_items 
